...
class StubSodaInstaller final : public SodaInstaller {
 public:
  ...
  // Empty, so that `SpeechRecognition.install()` is refused outright by
  // `IsLanguageInstallable`. An install this class committed to would park its
  // reply forever, because nothing here ever reports an outcome.
  ...
  // `LiveCaptionController` calls in from the caption UI.
