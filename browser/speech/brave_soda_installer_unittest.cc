...
// nothing changed, and the component updater is left unwatched because the
// early return comes before the observation.
...
// asked for, and only once however many requests are made. Watching from the
// constructor would build the component updater inside
// BrowserProcessImpl::PreMainMessageLoopRun, earlier than upstream does, and
// watching twice would report one failed download as two failures.
TEST_F(BraveSodaInstallerUnitTest, ObservesOnceFromInstallLanguage) {
  EXPECT_CALL(*cus_, AddObserver(testing::_)).Times(0);
  CreateInstaller();
  testing::Mock::VerifyAndClearExpectations(cus_);
  ...
  EXPECT_CALL(*cus_, RegisterComponent(testing::_))
      .WillRepeatedly(testing::Return(true));
  EXPECT_CALL(on_demand_updater_, EnsureInstalled(ComponentId(), testing::_))
      .WillRepeatedly([&requests]() { ++requests; });

  InstallLanguage();
  ASSERT_TRUE(base::test::RunUntil([&requests]() { return requests == 1; }));
  InstallLanguage();
  ASSERT_TRUE(base::test::RunUntil([&requests]() { return requests == 2; }));
}
