...
public class HttpsUpgradeTabHelper {
  ...
  private func shouldUpgradeToHttps(url: URL, isPrivate: Bool) -> Bool {
    guard FeatureList.kBraveHttpsByDefault.enabled,
      let httpsUpgradeService = HttpsUpgradeServiceFactory.get(privateMode: isPrivate),
  ...
  private func handleInvalidHTTPSUpgrade(tab: some TabState, responseURL: URL) -> URLRequest? {
...
extension HttpsUpgradeTabHelper: TabPolicyDecider {
  public func tab(
      ...
      existingUpgradeRequestURL == requestURL
    {
      ...
      guard let fallbackRequest = handleInvalidHTTPSUpgrade(tab: tab, responseURL: requestURL)
      else {
        return .allow
      }
    ...
    upgradeTimeoutTimer = Timer.scheduledTimer(
      ...
      block: { [weak self, weak tab] _ in
        guard let self, let tab,
          let fallbackRequest = self.handleInvalidHTTPSUpgrade(tab: tab, responseURL: upgradedURL)
        else { return }
        tab.stopLoading()
        tab.loadRequest(fallbackRequest)
      }
...
extension HttpsUpgradeTabHelper: TabObserver {
  ...
  public func tab(_ tab: some TabState, didFailNavigationWithError error: any Error) {
    ...
    guard let url = error.userInfo[NSURLErrorFailingURLErrorKey] as? URL,
      ...
      let fallbackRequest = handleInvalidHTTPSUpgrade(tab: tab, responseURL: url)
    else {
      return
    }
